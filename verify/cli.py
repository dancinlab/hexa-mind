#!/usr/bin/env python3
"""hexa-mind verify/cli.py — unified verifier dispatcher."""
...
CHECKS = [
    ("n6",        "n6_arithmetic.py",  "n=6 lattice + speculation honesty"),
    ("inventory", "spec_inventory.py", "7-verb spec presence (3 grounded + 4 speculative)"),
]
...
def _run_one(name, script, label):
    s = VERIFY_DIR / script
    if not s.exists():
        return {"name": name, "label": label, "ok": False, "exit_code": 2,
                "duration_s": 0.0, "stdout": "", "stderr": f"missing: {s}"}
    t0 = time.monotonic()
    rc = subprocess.run([sys.executable, str(s)], capture_output=True, text=True, cwd=str(ROOT))
    return {"name": name, "label": label, "ok": rc.returncode == 0,
            "exit_code": rc.returncode, "duration_s": round(time.monotonic()-t0, 3),
...
def main(argv):
    ...
    if a.json:
        ...
        print(json.dumps({"tool": "hexa-mind verify/cli.py", "ok": n_ok == len(results),
