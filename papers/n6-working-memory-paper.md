<!-- @canonical-origin: canon@a86ca143:papers/n6-working-memory-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: working-memory
requires: []
---
# [CANONICAL v2] Ultimate working memory (HEXA-WORKING-MEMORY) — n=6 arithmetic coordinate mapping

> **Author**: Minwoo Park (canon)
> **Category**: working-memory — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-427, BT-372, BT-427, BT-427, BT-427
> **Linked atlas node**: `working-memory` 0/24 EXACT [10*]

---

## 0. Abstract

This paper demonstrates, as a candidate framing, that the core parameters of the working memory domain
...
values of working memory. atlas.n6 records 0/24 items as EXACT.

This paper does not claim a new working memory; rather, it is a seed paper that overlays an **n=6
...
working memory is re-read within the n=6 arithmetic system. The perfect number n=6 simultaneously
satisfies the number-theoretic constant group σ(6)=12, τ(6)=4, φ=2, sopfr(6)=5, and this is
structurally aligned with the core parameters of the working memory domain. **This paper overlays an
n=6 arithmetic coordinate system onto the existing knowledge of working memory.**

| Effect | Baseline | After HEXA-WORKING-MEMORY | Experienced change |
|--------|----------|------------------------|--------------------|
...
pattern that meshes necessarily with the basic numerical values of working memory.

### What the n=6 coordinate mapping changes

```
  Before: "Why is this working memory value this number?" → experience / convention
  HEXA:   "This working memory value = σ(6) or τ(6) or sopfr(6)" → number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline working memory vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline working memory method vs HEXA-WORKING-MEMORY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-WORKING-MEMORY    system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a common parameter across dozens of domains beyond working memory.
...
Stage-wise maturation roadmap for HEXA-WORKING-MEMORY — verification density increases per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: integrate the entire working memory domain into n=6 arithmetic. Cross-reference with 295
...
Map core working memory parameters onto σ/τ/φ/sopfr/J₂.
...
Log-log regress the main scaling laws of working memory to check whether they follow the τ(6)=4 or
sopfr(6)=5 exponent.
...
Domain-specific physical upper bounds (Carnot/Shannon/Bekenstein, etc.) checked separately for working memory.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-WORKING-MEMORY n=6 honesty verification (stdlib only, working-memory domain)
#
...
FALSIFIERS = [
    "If main working memory parameter n=6 alignment < 70%, retract the core claim of this paper",
